 ...
 * @author Matse
 */
class LayerOpenFLStarlingEditable extends EventDispatcher implements ITimeLineLayerEditable implements ILayerOpenFLEditable implements ILayerStarlingEditable
{
	static private var _POOL:Array<LayerOpenFLStarlingEditable> = new Array<LayerOpenFLStarlingEditable>();
	
	static public function fromPool():LayerOpenFLStarlingEditable
	{
		if (_POOL.length != 0) return _POOL.pop();
		return new LayerOpenFLStarlingEditable();
	}
	...
	private function set_locked(value:Bool):Bool
	{
		...
		this._displayContainer.mouseEnabled = this._displayContainerStarling.touchable = !value;
		this._locked = value;
	...
	**/
	public function indexUpdate(newIndex:Int):Void
	{
		...
		if (this._rootContainer != null)
		{
			...
			{
				this._rootContainer.setChildIndex(this._displayContainer, this._index);
			}
		}
	...
	public function activate(object:ValEditorObject):Void 
	{
		...
		else if (object.isContainer)
		{
			...
			{
				cast(object.object, IContainerOpenFLEditable).rootContainer = this._displayContainer;
			}
