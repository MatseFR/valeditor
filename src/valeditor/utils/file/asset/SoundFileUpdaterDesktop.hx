...
class SoundFileUpdaterDesktop 
{
	...
	public function start(asset:SoundAsset, file:File, completeCallback:Void->Void):Void
	{
		this._asset = asset;
		this._file = file;
		...
		Sound.loadFromFile(this._file.nativePath).onComplete(onSoundLoadComplete).onError(onSoundLoadError);
	}
	
	private function onSoundLoadComplete(sound:Sound):Void
	{
		ValEdit.assetLib.updateSound(this._asset, this._file.nativePath, sound, this._bytes);
	...
	{
		trace("SoundFileUpdaterDesktop failed to load " + this._file.nativePath);
