	...
	**/
	static public function initialize(theme:SimpleTheme, styleProvider:ClassVariantStyleProvider):Void
	{
		...
		if (styleProvider.getStyleFunction(Button, null) == null) {
			styleProvider.setStyleFunction(Button, null, Default);
		}
		
		if (styleProvider.getStyleFunction(Button, Button.VARIANT_PRIMARY) == null) {
			styleProvider.setStyleFunction(Button, Button.VARIANT_PRIMARY, primary);
		}
		
		if (styleProvider.getStyleFunction(Button, Button.VARIANT_DANGER) == null) {
			styleProvider.setStyleFunction(Button, Button.VARIANT_DANGER, danger);
		}
		
		if (styleProvider.getStyleFunction(Button, ButtonVariant.DIVIDER_H) == null)
		{
			styleProvider.setStyleFunction(Button, ButtonVariant.DIVIDER_H, divider_h);
		}
		
		if (styleProvider.getStyleFunction(Button, ButtonVariant.DIVIDER_V) == null)
		{
			styleProvider.setStyleFunction(Button, ButtonVariant.DIVIDER_V, divider_v);
		}
	}
	
	static private function Default(button:Button):Void
	{
	...
	static private function danger(button:Button):Void
	{
		if (button.backgroundSkin == null) {
			var skin = new RectangleSkin();
	...
	static private function divider_h(button:Button):Void
	{
		if (button.backgroundSkin == null)
		{
			var skin = new LeftAndRightBorderSkin();
			skin.fill = theme.getLightFillLight();
			skin.disabledFill = theme.getLightFillDark();
			skin.setFillForState(DOWN, theme.getThemeFill());
			skin.setFillForState(HOVER, theme.getThemeFillLight());
	...
	static private function divider_v(button:Button):Void
	{
		if (button.backgroundSkin == null)
		{
			var skin = new TopAndBottomBorderSkin();
			skin.fill = theme.getLightFillLight();
			skin.disabledFill = theme.getLightFillDark();
			skin.setFillForState(DOWN, theme.getThemeFill());
			skin.setFillForState(HOVER, theme.getThemeFillLight());
	...
	static private function primary(button:Button):Void
	{
		if (button.backgroundSkin == null) {
			var skin = new RectangleSkin();
