 ...
 * @author Matse
 */
class ValEditorStarter 
{
	private var _completeCallback:Void->Void;
	private var _exposeDataFunction:Void->Void;
	...
	public function start(completeCallback:Void->Void, exposeDataFunction:Void->Void = null, root:DisplayObjectContainer = null, simple:Bool = false):Void
	{
		this._completeCallback = completeCallback;
		this._exposeDataFunction = exposeDataFunction;
		
		ValEditor.theme = new ValEditorTheme();
		Theme.setTheme(ValEditor.theme, root);
		
		if (simple)
		{
			ValEditor.initSimple(initComplete);
		}
		else
		{
			ValEditor.init(initComplete);
		}
	}
	...
	{
		if (this._exposeDataFunction != null)
		{
			this._exposeDataFunction();
		}
		
		// UI Theme
		if (!ValEditor.hasClass(ValEditorTheme))
		{
			ValEditor.registerClassSimple(ValEditorTheme, false, SimpleThemeData.exposeSimpleTheme());
		}
		
		// Editor Settings
		if (!ValEditor.hasClass(EditorSettings))
		{
